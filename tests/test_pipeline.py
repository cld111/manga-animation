...
@requires_ffmpeg
def test_run_pipeline_end_to_end_with_speed_lines_mesh_warp_primary(
    page_path: Path, config, tmp_path: Path
):
    """Phase 16: a `speed_lines` PRIMARY (the flow-class drawn effect, mapped to MESH_WARP by
    `_EFFECT_LABEL_KEYWORDS`) must run the whole pipeline end to end and produce a playable
    video -- the mesh_warp effect path is not just the analysis signal, it reaches the
    transform inside the orchestrator.
    """
    from manga_animation.schemas.animation_plan import TransformKind
    ...
    assert isinstance(result, PipelineRunResult)
    assert result.primary_object.motion_type == MotionType.PRIMARY
    assert result.primary_object.semantic_label == "speed_lines"
    assert result.primary_object.motion is not None
    assert result.primary_object.motion.transform_kind == TransformKind.MESH_WARP
    assert result.render.output_path.exists()
    assert result.render.frame_count == config.fps * config.duration_s

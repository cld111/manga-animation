"""Deterministic prompt construction: Qwen object descriptions -> Wan2.2 prompt.
...
def build_animation_prompt(
    ...
    """Compose the Wan2.2 text prompt from accepted objects, PRIMARY first.

    `objects` must already be the accepted, ranked list (plan order, PRIMARY first). The
    prompt is `"<primary phrase>, <secondary phrase>, ..."` -- a flat comma-joined instruction
    whose first element is the readable PRIMARY action. Empty input raises `ValueError` (fail
    closed; the caller must never feed an empty acceptance into the generative engine).
    """
    if not objects:
        raise ValueError("cannot build an animation prompt from an empty object list")
    ordered = sorted(
        objects,
        key=lambda item: (

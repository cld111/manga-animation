"""Generative animation engine: Alibaba AnimateAnything (motion-guidance video diffusion).

Since the 2026 architecture change the engine turns (DINO bbox crop, Qwen description prompt)
into a short generated video -- no SAM segmentation, no motion mask. It is THE animation
engine for this integration: grounding (DINO) -> object description (Qwen) ->
AnimateAnything(image crop, prompt) -> per-object MP4. The deterministic OpenCV transforms
remain in `manga_animation.animation` for the legacy path, but a run that selects
...
serializes a spec (image, prompt, hyper-parameters), invokes `worker.py` with the isolated
interpreter, and reads the generated frames back. Local work stays code/tests/config; the
heavy model and its pinned environment are remote-GPU concerns (ADR 0003).

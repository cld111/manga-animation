...
`animate()` takes the per-object DINO bbox crop and the prompt built from the accepted Qwen
description -- no SAM segmentation, no motion mask (the whole crop may move). This is the
2026 architecture integration contract: grounding (DINO) -> object description (Qwen) ->
AnimateAnything(image crop, prompt) -> per-object MP4.
"""
...
from manga_animation.pipeline.types import FrameSequence, ImageArray
...
class AnimateAnythingClient:
    """Subprocess-backed AnimateAnything inference client (image + prompt -> frames).
    ...
    def animate(
        ...
        """Generate one object's animation from (image crop, prompt) and return the frames.

        `image` is the DINO bbox crop of the accepted object; `prompt` comes from
        `build_animation_prompt` for that object's accepted Qwen description. No motion mask:
        the whole crop is the motion area. Writes the spec + input into `out_dir`, runs the
        worker, reads the frame PNGs back. Raises `subprocess.CalledProcessError`/
        `FileNotFoundError` on worker failure (fail closed).
        """

...
class PipelineConfig(BaseModel):
    ...
    # AnimeGen-I2V generative animation engine (ADR 0024). These are the hyper-parameters
    # handed to the isolated worker for the model's native output (81 frames @ 16 fps = 5s).
    animation_fps: int = Field(gt=0, le=60, default=16)
    animation_num_frames: int = Field(gt=0, default=81)
    animation_num_inference_steps: int = Field(gt=0, default=4)
    animation_guidance_scale: float = Field(gt=0.0, default=1.0)
